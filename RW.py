...
def multiview_IRWK(Garr):
	...
	for i in range(views):
		...
		Pt[i] = np.array([((int(Garr[i].has_edge(x1, x)) * (1 - stop_prob - trans_prob * (views - 1)))+1) / (len(list(Garr[i].neighbors(x)))+1) for x in nodes[i] for x1 in nodes[i]])
		Pt[i].shape = shape

		r1[i] = np.array([sum([Pt[i][x1, x] * R0[i][x1, y1] * Pt[i][y1, y]  for x1 in range(len(nodes[i])) for y1 in range(len(list(nodes[i])))]) for x in range(len(list(nodes[i]))) for y in range(len(list(nodes[i])))])
		r1[i].shape = shape
